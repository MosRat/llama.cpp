...
struct clip_graph {
    ...
    ggml_cgraph * build_got_vary() {
        ...
        constexpr int64_t GOT_VARY_PATCH_SIZE = 64;
        ...
        ggml_tensor * inp_raw = build_inp_raw();
        ggml_tensor * inp     = ggml_conv_2d_sk_p0(ctx0, model.patch_embeddings_0, inp_raw);
        ...
        inp = ggml_add_inplace(ctx0, inp, model.patch_bias);

        ggml_tensor * cur = ggml_add_inplace(ctx0, inp, model.position_embeddings);
        ...
        for (int il = 0; il < n_layer; il++) {
            ...
            const int64_t n_patches = full_attn ? 4096 : 196;
            ...
            // self-attention
            {
                ...
                Qcur = ggml_reshape_4d(ctx0, Qcur, d_head, n_head, n_patches, B);
                Kcur = ggml_reshape_4d(ctx0, Kcur, d_head, n_head, n_patches, B);
                Vcur = ggml_reshape_4d(ctx0, Vcur, d_head, n_head, n_patches, B);

                cb(Qcur, "Qcur", il);
                cb(Kcur, "Kcur", il);
                cb(Vcur, "Vcur", il);

                ggml_tensor * rel_w = layer.rel_w;
                ggml_tensor * rel_h = layer.rel_h;

                cur = build_attn_for_got_vary(layer.o_w, layer.o_b, Qcur, Kcur, Vcur, rel_w, rel_h, H, W, B, kq_scale,
                                              il);
                cb(cur, "attn_out", il);
            }
        ...
        cur = ggml_conv_2d_sk_p0(ctx0, model.neck_conv1_w, cur);
        cur = buidl_norm_2d_for_got_vary(ctx0, cur, GOT_NORM_CHANS, model.neck_norm1_w, model.neck_norm1_b, eps);
        cur = ggml_conv_2d_s1_ph(ctx0, model.neck_conv2_w, cur);
        cur = buidl_norm_2d_for_got_vary(ctx0, cur, GOT_NORM_CHANS, model.neck_norm2_w, model.neck_norm2_b, eps);
  ...
  private:
    ...
    struct ggml_tensor * buidl_norm_2d_for_got_vary(struct ggml_context * ctx0, struct ggml_tensor * layer,
                                                    int n_channels, struct ggml_tensor * w, struct ggml_tensor * b,
                                                    float eps) {
        // LayerNorm2d
    ...
    ggml_tensor * build_attn_for_got_vary(ggml_tensor * wo, ggml_tensor * wo_b, ggml_tensor * q_cur,
                                          ggml_tensor * k_cur, ggml_tensor * v_cur, ggml_tensor * rel_w,
                                          ggml_tensor * rel_h, int64_t H, int64_t W, int64_t B, float kq_scale,
                                          int il) const {
        // these nodes are added to the graph together so that they are not reordered
        // by doing so, the number of splits in the graph is reduced
        ggml_build_forward_expand(gf, q_cur);
        ...
        ggml_tensor * q = ggml_cont(ctx0, ggml_permute(ctx0, q_cur, 0, 2, 1, 3));
        //cb(q, "q", il);
...
struct llava_uhd {
    ...
    static slice_instructions get_slice_instructions(struct clip_ctx * ctx, const clip_image_size & original_size) {
        ...
        if (has_pinpoints) {
            ...
            auto refine_size =
                llava_uhd::select_best_resolution(ctx->model.hparams.image_grid_pinpoints, original_size);
...
int clip_n_mmproj_embd(const struct clip_ctx * ctx) {
    ...
    switch (ctx->model.proj_type) {
        ...
        case PROJECTOR_TYPE_GOTVARY:
            return ctx->model.proj_lin_w->ne[1];
        default:
            GGML_ABORT("Unknown projector type");
    }
}
